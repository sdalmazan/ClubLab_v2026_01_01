// ============================================================
// scrape-juvenil-regional.ts — Scraper de Juvenil Regional (Playwright Headless)
// ============================================================
// Scrapea e inserta en la base de datos de estadísticas y actualiza la caché
// de scouting para los dos grupos de Juvenil Regional en la temporada 2025/2026.
//
// Uso:
//   npx tsx scripts/scrape-juvenil-regional.ts
// ============================================================
...
async function main() {
  console.log("════════════════════════════════════════════════════════");
  console.log("  Scraper de Juvenil Regional (Castilla y León)");
  console.log("  Temporada 2025/2026 | Grupos A y B");
  console.log("════════════════════════════════════════════════════════");
  ...
  // Definir las competiciones a scrapear
  const targets = [
    { season: "2025/2026", competition: "Juvenil Regional - Grupo A" },
    { season: "2025/2026", competition: "Juvenil Regional - Grupo B" },
  ];

  const scraperOptions = {
    delayMatch: 8000,       // 8 segundos de pausa entre descargas de actas
    delayMatchday: 3000,    // 3 segundos de pausa entre jornadas
    maxMatchday: 34,        // 34 jornadas de límite
    emptyLimit: 4,          // Detenerse tras 4 jornadas vacías consecutivas
  };

  for (let idx = 0; idx < targets.length; idx++) {
    const target = targets[idx];
    console.log(`\n[Grupo JR ${idx + 1}/${targets.length}] Iniciando ${target.competition} (${target.season})...`);
  ...
  console.log("\n════════════════════════════════════════════════════════");
  console.log("  PROCESO DE SCRAPING DE JUVENIL REGIONAL COMPLETADO");
  console.log("════════════════════════════════════════════════════════\n");
  process.exit(0);
}
