  ...
  try {
    ...
    if (sessionIds.length > 0) {
      const { data: allMetrics } = await supabase
        .from("wimu_player_session_metrics")
        .select("player_id, distance_km, hsr_m, sprints_count, max_speed_kmh, player_load_min, accelerations")
      ...
      if (allMetrics && Array.isArray(allMetrics)) {
        ...
        allMetrics.forEach((m) => {
          ...
          if (!pid) return;
          if (!acc[pid]) acc[pid] = { count: 0, dist: 0, hsr: 0, sprints: 0, maxSpeed: 0, plMin: 0, accel: 0 };
          acc[pid].count  += 1;
