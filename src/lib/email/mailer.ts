...
export async function sendOtpEmail({
  ...
  recipientName?: string;
  otpCode: string;
  purpose?: "onboarding" | "channel_change" | "security";
}) {
  if (!to || !to.includes("@")) return false;

  const purposeText =
    purpose === "channel_change"
      ? "verificar tu nuevo canal de notificaciones"
      : purpose === "security"
      ? "confirmar una acción de seguridad"
      : "completar tu registro en ClubLab";
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="utf-8">
      <title>Código de Verificación - ClubLab</title>
    </head>
    <body style="margin: 0; padding: 24px 12px; background-color: #0f172a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #f8fafc;">
      <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 520px; background-color: #1e293b; border: 1px solid #334155; border-radius: 16px; padding: 32px 24px; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.5);">
        <tr>
          <td>
            <div style="font-size: 24px; font-weight: 800; color: #3b82f6; margin-bottom: 20px; text-align: center;">
              Club<span style="color: #ffffff;">Lab</span>
            </div>
            
            <h2 style="font-size: 18px; font-weight: 700; color: #ffffff; margin: 0 0 12px 0; text-align: center;">
              Código de Verificación OTP
            </h2>

            <p style="font-size: 14px; color: #94a3b8; line-height: 1.6; margin-bottom: 24px; text-align: center;">
              Hola ${recipientName}, utiliza el siguiente código de 6 dígitos para <strong>${purposeText}</strong>:
            </p>

            <div style="background-color: #0f172a; border: 2px dashed #3b82f6; border-radius: 12px; padding: 18px; text-align: center; margin-bottom: 24px;">
              <span style="font-family: monospace; font-size: 32px; font-weight: 900; letter-spacing: 8px; color: #10b981;">
                ${otpCode}
              </span>
            </div>

            <p style="font-size: 12px; color: #64748b; line-height: 1.5; margin-bottom: 16px; text-align: center;">
              ⚠️ <strong>Este código es válido por 10 minutos.</strong> Nunca compartas este código con nadie por razones de seguridad.
            </p>

            <div style="border-top: 1px solid #334155; padding-top: 16px; font-size: 11px; color: #64748b; text-align: center;">
              Si no has solicitado este código, puedes ignorar este correo con total seguridad.
            </div>
          </td>
        </tr>
      </table>
  ...
  try {
    await transporter.sendMail({
      ...
      to,
      replyTo: defaultReplyTo,
      subject: `[ClubLab] Tu código de verificación: ${otpCode}`,
      text: `Hola ${recipientName},\n\nTu código de verificación de 6 dígitos es: ${otpCode}\n\nEs válido por 10 minutos.`,
      html: htmlContent,
    });
    console.log(`[sendOtpEmail] Real OTP email sent to ${to}`);
    return true;
  } catch (err: any) {
    console.error("[sendOtpEmail] Error sending email:", err.message);
    return false;
  }
}
