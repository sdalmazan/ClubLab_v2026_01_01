...
function matchPlayerName(lineupName: string, eventName: string): boolean {
  if (!lineupName || !eventName) return false;
  
  const ignored = new Set(["de", "del", "la", "las", "el", "los", "y", "da", "do"]);
  ...
  const lineupWords = clean(lineupName).filter(w => !ignored.has(w));
  const eventWords = clean(eventName).filter(w => !ignored.has(w));

  if (lineupWords.length === 0 || eventWords.length === 0) return false;

  for (const ew of eventWords) {
    if (ew.length >= 3) {
      if (!lineupWords.includes(ew)) return false;
    } else {
      const matchInitial = lineupWords.some(lw => lw.startsWith(ew));
      if (!matchInitial) return false;
    }
  }
  return true;
}
...
export async function saveMatchTransactional(
  ...
  const { data: insertedMatch, error: matchError } = await supabase
    ...
    .insert({
      ...
      local_staff: parsed.local_staff ?? null,
      visitor_staff: parsed.visitor_staff ?? null,
    })
    .select("id")
    .single();
...
async function savePlayerInfluenceWithSeason(
  ...
  for (const lineup of lineups) {
    ...
    const goalsScored = parsed.goals.filter(
      (g) =>
        g.type === "normal" &&
        matchPlayerName(lineup.player_name, g.player_name) &&
        (g.team === "local") === isHome
    ).length;
    ...
    const yellowCards = parsed.cards.filter(
      (c) => c.type === "amarilla" && matchPlayerName(lineup.player_name, c.player_name)
    ).length;

    const redCards = parsed.cards.filter(
      (c) =>
        (c.type === "roja" || c.type === "doble amarilla") &&
        matchPlayerName(lineup.player_name, c.player_name)
    ).length;
